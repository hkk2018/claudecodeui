...
const formatTimeAgo = (dateString: any, currentTime: any) => {
  ...
  const diffInMs = (now instanceof Date ? now.getTime() : now) - date.getTime();
...
function Sidebar({
  ...
  const saveToRecentPaths = (path: string) => {
    try {
      const recent = JSON.parse(localStorage.getItem('recentProjectPaths') || '[]');
      const updated = [path, ...recent.filter((p: string) => p !== path)].slice(0, 10);
      localStorage.setItem('recentProjectPaths', JSON.stringify(updated));
    } catch (error) {
  ...
  const updateSessionSummary = async (sessionId: string, summary: string) => {
    try {
      await api.updateSession(sessionId, { summary });
  ...
  const getAllSessions = (project) => {
    ...
    return [...claudeSessions, ...cursorSessions].sort((a, b) => normalizeDate(b).getTime() - normalizeDate(a).getTime());
  };
                    ...
                    <div className="ml-3 space-y-1 border-l border-border pl-3">
                      {!initialSessionsLoaded.has(project.name) ? (
                        ...
                        getAllSessions(project).map((session) => {
                          ...
                          const diffInMinutes = Math.floor(((currentTime instanceof Date ? currentTime.getTime() : currentTime) - sessionDate.getTime()) / (1000 * 60));
                                  ...
                                  <>
                                    <input
                                      ...
                                      onKeyDown={(e) => {
                                        e.stopPropagation();
                                        if (e.key === 'Enter') {
                                          updateSessionSummary(session.id, editingSessionName);
