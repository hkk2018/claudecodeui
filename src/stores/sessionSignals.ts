 ...
 */
export const sessionCache = signal<SessionCache>({});

/**
 * LRU 存取順序（最近使用的在最後）
 */
export const accessOrder = signal<string[]>([]);
 ...
 */
export const sessionProcessingState = signal<SessionProcessing>({});
 ...
 * 設定 session 的消息（覆蓋）
 */
export function setSessionMessages(sessionId: any, messages: any, pagination: any = {}) {
  sessionCache.value = {
    ...sessionCache.value,
    [sessionId]: {
      messages,
 ...
 * 前置舊消息（載入更多）
 */
export function prependSessionMessages(sessionId: any, messages: any, pagination: any = {}) {
 ...
 * 更新 pagination
 */
export function updatePagination(sessionId: any, pagination: any) {
  const existing = sessionCache.value[sessionId];
  if (!existing) return;
