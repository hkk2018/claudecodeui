...
last_modified: "2026-06-01 05:31"
---
...
- **`/api/hook-event` Stop handler**（commit `89fb591`）：原本 `readFile(transcript_path,'utf-8')` + `trim().split('\n')` 只為拿最後一則 assistant 訊息。**這個 hook 每個 turn 結束都觸發、8 個 IDE 的 AI PM loop 狂發 → 是最頻繁也最致命的觸發點**。
- **`/api/.../token-usage`**（commit `89fb591`）：同樣整檔讀，只為取最後一筆含 usage 的 assistant entry（開啟 session 時呼叫）。

後兩條抽出共用的 `findLastEntryFromTail(path, predicate)`（成長式尾讀），兩處共用。

## 第二個教訓：單發測試會騙人，要 soak test

前兩條（getSessions / getSessionMessages）我**單發請求都測過、都通過**（0.07s、70MB），但部署後 6 分鐘 soak test 在第 3 分鐘又爆——因為 **Stop hook 這條只在「真實 turn 結束」時才會被打**，單發 API 測試永遠碰不到。是 soak（持續觀察記憶體 + 計數 Stop hook）才把這條最頻繁的揪出來。

**方法論**：這類「間歇性 OOM / 資源洩漏」必須用**持續負載下的時間窗觀測**驗收，不能只看單發請求的數字。同一類 bug（整檔讀）往往散在多條路徑，要一次掃乾淨（見下方殘留），並用 soak 確認真實流量下不復發。

## 驗證結果（完整修復後，6 分鐘 soak）

| 指標 | 修復前 | 完整修復後 |
|------|--------|-----------|
| onexas `sessions?limit=1`（575MB 檔）| 4.7s | 0.074s |
| onexas `messages?limit=30`（208k 則）| 2.7s | 0.084s |
| Stop hook 打 575MB transcript | 整檔讀 → OOM | 0.055s |
| token-usage on 575MB | 整檔讀 → OOM | 0.081s |
| dev service CPU | 29.6%（榜首）| 3.8% |
| 記憶體（有 client poll + 4 次 Stop hook）| 爬到 ~448MB 撞 1G 爆 | 全程 57–81MB 平穩 |
| OOM | 今日 50 次 / crash loop | 6 分鐘 soak 0 次、PID 不變 |

小檔專案（claudecodeui）的 summary 與精確 messageCount 不變，無回歸。

## 已知殘留 / 後續

- **`deleteSession`（`server/projects.ts:1372`）仍整檔讀**：刪 session 時 `readFile` 整檔來過濾要保留的行。屬手動刪除的罕見路徑、且本質要重寫檔案，暫留；若對巨檔刪 session 會 OOM，再改成串流重寫。
- **估算值偏差**：messageCount badge 與大檔 messages 的 `total` 都是估算，目前一個偏高（用檔頭密度）、一個偏低（用檔尾密度），純顯示用、不影響功能。若要一致可改成 head+tail 雙取樣平均。
- **cgroup 1G 不需調大**：拿掉大檔全讀後，含 poll + Stop hook 的實際用量 57–81MB，1G 非常寬裕。
